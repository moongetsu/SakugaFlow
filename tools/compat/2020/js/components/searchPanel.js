  ...
  const SearchPanel = {
    ...
    renderSuggestions() {
      ...
      this.currentSuggestions.forEach((tag, index) => {
        ...
        const handler = (event) => {
          if (window.Downloader && window.Downloader.debugLog) {
            window.Downloader.debugLog(`Suggestion event triggered: ${event.type} for tag: ${tag.name}`);
          }
        ...
        item.addEventListener("mouseup", handler);
    ...
    applySuggestion(tagName) {
      if (window.Downloader && window.Downloader.debugLog) {
        window.Downloader.debugLog(`applySuggestion start. tagName: ${tagName}`);
      }
      try {
        ...
        const suffix = afterToken.replace(/^\s*,\s*/, "").replace(/^\s+/, "");
        ...
        if (window.Downloader && window.Downloader.debugLog) {
          window.Downloader.debugLog(`applySuggestion crashed: ${err.message}\nStack: ${err.stack}`);
        }
      }
    },
